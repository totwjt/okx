...
class GridLsV1Strategy(IStrategy):
    ...
    volume_ratio_threshold = DecimalParameter(0.5, 2.5, default=0.9, decimals=2, space="buy")
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        short_condition = (  (   (dataframe['adx'] <= 23) &   (dataframe['close'] >= dataframe['bb_upper'] * 0.998) &   (dataframe['zscore'] >= 0.55) &   (dataframe['rsi'] >= rsi_overbought)  ) |  (   (dataframe['ema_fast'] < dataframe['ema_slow']) &   (dataframe['zscore'] >= 0.9) &   (dataframe['atr_pct'] <= 0.032)  ) ) & (dataframe['volume_ratio'] >= volume_ratio_threshold) 
